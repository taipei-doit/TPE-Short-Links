...
- **有效期限不得超過目標網域的註冊到期日（防呆）**：建立時以 RDAP 查目標網域（`gov.taipei`、`example.com` 這一層）的註冊到期日並記在後台；指定的到期時間晚於它會被擋下，「永久有效」照常可建但顯示警語、列表標「超過網域期限」。短網址的到期日是自己的，不隨網域連動、不會自動延長：網域續約後在管理頁按「重新查詢網域」更新上限（`scripts/refresh_domains.py` 可批次做），再手動延長短網址。註冊機構不公開到期日的網域（`gov.tw`、`edu.tw`、`.jp` 等）不設上限
- **疑似易主防護**：RDAP 只說「這筆登記到什麼時候」，不說登記人是誰，所以後台另記網域的註冊日期、註冊商、DNS。重查時若註冊日期比記錄的晚（到期後被他人重新註冊會重設此日期）、或網域從註冊機構消失，新資料一律擱置不採用，列表標紅「疑似已易主」，並禁止建立或改指向該網域的短網址；管理員在確認視窗看過新舊差異、按下「確認此網域仍為本機關所有」後才採用（記錄確認人與時間）。同一擁有者續約時註冊日期不變，直接更新
...
下載一律走後端串流，這樣儲存桶才能保持私有、PIN 也才是唯一的關卡。

**注意：32 MiB 的限制同時適用於回應。** 回應只有在「分塊傳輸」（chunked）時才不受限，而只要帶了 `Content-Length` 就會被當成固定長度回應、整包緩衝並套用上限。因此超過 30 MB（`RESPONSE_LENGTH_LIMIT`）的檔案**刻意不送 `Content-Length`**，代價是瀏覽器下載時看不到百分比；30 MB 以下維持固定長度以保留進度顯示。這一點有回歸測試 `test_large_downloads_omit_content_length` 守著。

診斷用：`scripts/check_download.py` 會用服務自己的密鑰簽出下載權杖（不需 PIN），把最大的檔案從正式網址抓一次並比對長度——只有真的走一趟公開邊緣才驗得到這個限制，儲存層的檢查抓不到。
...
| `POST /api/domains/{name}/confirm` | 管理員確認「疑似易主」的網域仍為本機關所有，採用擱置中的新登記資料 |
